...
echo 'Grabbing and building OpenSSL 1.0.1'
wget http://www.openssl.org/source/openssl-1.0.1e.tar.gz
tar -xzf openssl-1.0.1e.tar.gz
cd openssl-1.0.1e
mkdir build
./config  --prefix=$(pwd)/build -fPIC
make
make install
cd ..

echo 'Grabbing the required versions of Python, P4Python, and the P4API...'
wget --no-check-certificate http://www.python.org/ftp/python/3.3.2/Python-3.3.2.tar.bz2
...
cd p4python-*
export PATH=.:$PATH
/usr/local/bin/python3 setup.py build --apidir ../p4api-2014.1.*/ --ssl ../openssl-1.0.1e/build/lib

sudo /usr/local/bin/python3 setup.py install --apidir ../p4api-2014.1.*/ --ssl ../openssl-1.0.1e/build/lib
...
mkdir -p build
cd build
/usr/local/bin/cmake -DBUILD_CLAR=off ..
