...
from   p4gf_g2p_matrix_row          import G2PMatrixRow    as Row
from   p4gf_g2p_matrix_row          import G2PMatrixCell   as Cell
from   p4gf_g2p_matrix_row          import Decided
...
DO_UNEXPAND_KTEXT             = 'unexpand -k RCS keywords'
...
class G2PMatrix:
    ...
    GPARN    = Intersection of one Git commit parent and one Git Fusion branch.
               One for each such Git Fusion branch. Can be lightweight or
               fully populated.
            ...
            if self.is_linear_fp():
                ...
                for row in self.rows_sorted:
                    if not row_decider.decide(row):
                        ...
                        gdc = row.cells[self._gdest_column.index]
                        if gdc and gdc.discovered and gdc.discovered.get('git-action') == 'D':
    ...
    def do_it(self):
        ...
        with Timer(DO_IT):
            ...
            if self.is_linear_fp():
                # +++ Linear fully populated history does not need to waste time
                # +++ or network I/O to sync, integ, or resolve any files. Skip
                self._do_create_local_placeholder_if()
                            ...
                            # which can prevent later integ for branch or edit.
                            # Get these done and resolved and off our local
                            # filesystem.
                ...
                self._force_local_filesystem_empty(DO_IT_RM_RF)
                            ...
                            # just the files that have to exist locally to keep
                            # 'p4 integ' from failing with error. We don't
                            # _really_ need these files (Git has all the file
                            # content we need), but Perforce requires them.
                            # Can't be helped.
                self._do_p4_sync_f()
                self._do_create_local_placeholder_if()
                ...
                for col in self.columns:
                    self._do_integ(column = col, for_delete = False)
                            ...
                            # A little bit of post-integ discovery and deciding
                            # to react to whatever the Perforce server did
                            # for our integ+resolve requests.
                self._decide_p4_requests_post_do_integ()
                self._decide_p4filetypes_post_do_integ()
                self._unexpand_ktext()
    ...
    def ghost_decide(self):
        '''
        NOP stub. Implemented only in matrix2.
        '''
        pass
    ...
    def _discover_branches(self):
        ...
        with Timer(DISCOVER_BRANCHES):
            ...
            col_list = self._strip_none( [ gdest, p4jitfp, p4imply ]
                                       + gparn_list
                                       + gparfpn_list )
    ...
    def _discover_files(self):
        ...
        with Timer(DISCOVER_FILES):
            ...
            if not self.is_linear_fp():
                ...
                #self._populate_first_commit_on_current_branch()
    ...
    def _git_ls_tree(self, column, commit_sha1):
        ...
        for r in p4gf_util.git_ls_tree_r( repo         = self.ctx.view_repo
            ...
            if r.type != 'blob':
                _debug3('_git_ls_tree() skip {r}', r=r)
                continue
            _debug3('_git_ls_tree()      {}', r)
...
#    def _populate_first_commit_on_current_branch(self):
#        '''
#        Discovery.
#
#        Seed empty branches with the state of the world from their parent(s).
#
#        Seeding a lightweight branch copies only the files from its parent that
#        differ from fully populated Perforce.
#
#        Seeding a fully populated branch copies everything: parent's fully
#        populated basis + parent's differences from fully populated Perforce.
#        '''
...
#
#            if self.current_branch.is_lightweight:
#                self._populate_first_commit_lt()
#            else:
#                self._populate_first_commit_fp()

# was called _populate_first_commit_on_current_branch  from ebb3597
...
#    def _populate_first_commit_lt(self):
#        '''
#        Discovery.
#
#        Seed a new lightweight branch with files from its Git first parent.
#
#        Lightweight branches need only copy files that differ from the
#        lightweight branch's own fully populated basis.
#
#        Lightweight branches never populate from a fully populated first parent.
#        That's kind of the whole point of "lightweight" branches.
#        '''
#        LOG.debug('_populate_first_commit_lt()')
#        for gparn in (col for col in self.columns
#                      if (    col.is_first_parent
#                          and col.branch.is_lightweight
#                          and col.col_type == Column.GPARN)):
#            self._store_discovered_cells(gparn, self._copy_from(gparn))
#            self._store_discovered_cells(gparn, self._files_at (gparn))

#   was _populate_first_commit_lt  ... from ebb3597
    def _discover_files_populate_from(self):
        ...
        for gparn in (col for col in self.columns
            ...
            gparfpn = Column.find_fp_basis(self.columns, gparn)
...
#    def _populate_first_commit_fp(self):
#        '''
#        Discovery.
#
#        Seed a new fully populated branch with files from its Git first parent.
#
#        Include fully populated basis if first parent's branch is lightweight.
#        '''
#        LOG.debug('_populate_first_commit_fp()')
#        for gparn in (col for col in self.columns
#                      if (    col.is_first_parent
#                          and col.col_type == Column.GPARN)):
#            # Populate from parent.
...
#            self._store_discovered_cells(gparn, self._files_at (gparn))
#
#            # If parent is lightweight, populate from its fully populated basis.
#            gparfpn = Column.find_fp_basis(self.columns, gparn)
#            if gparfpn:
#                self._store_discovered_cells(gparfpn, self._copy_from(gparfpn))
#                self._store_discovered_cells(gparfpn, self._files_at (gparfpn))
    ...
    def _git_diff_tree(old_sha1, new_sha1):
        ...
        Need the -z flag so that paths are not munged and enquoted if they
        ...
        for pair in p4gf_util.pairwise(d['out'].split('\0')):
            parts = pair[0].split()
            if parts[0] == ':160000' or parts[1] == '160000':
    ...
    def _discover_p4imply_files(self):
        ...
        with Timer(DISCOVER_P4IMPLY):
            ...
            for r in self._git_diff_tree(prev_sha1, self.fe_commit['sha1']):
                row  = self._row(r.gwt_path)
                cell = row.cell(p4imply.index)
                cell.discovered = { 'git-action' : r.action }
                LOG.debug3('_discover_p4imply_files() {} {}'
    ...
    def _cell_p4_exists_at_gdest_head(self, row):
        ...
        if not (cell and cell.discovered):
            return False
        action = cell.discovered.get('action')
        if (   (not action)
            or ('delete' in action) ):
            return False
        return True
    ...
    def _requires_sync_f(self, row):
        ...
        exists_in_gdest = _gdest_cell_exists_in_p4(
    ...
    def _do_p4_sync_f(self):
        ...
        with Timer(SYNC_F):
            client_path_list = [self.ctx.gwt_path(row.gwt_path).to_client()
                                for row in self.rows_sorted
                                if self._requires_sync_f(row)]
            if not client_path_list:
                _debug2('_do_p4_sync_f() no files to sync')
                return

            _debug2('_do_p4_sync_f() ct={ct}', ct=len(client_path_list))
            _debug3('{l}', l=client_path_list)
    ...
    def _do_integ(self, column, for_delete):
        ...
        '''
        _debug2( '_do_integ() col={col} for_delete={for_delete}'
    ...
    def _dump_on_integ_failure(self, human_msg, row, column):
        ...
        try:
            ...
            with self.ctx.p4.at_exception_level(self.ctx.p4.RAISE_NONE):
                ...
                # Dump filelog history of afflicted row.
                cmd = ['filelog', '-i', src_depot_path, dest_depot_path]
                log.error('filelog of integ source and destination:\np4 {}'
                          .format(' '.join(cmd)))
                r = self.ctx.p4.run(cmd)
                log.error('\n{}'.format(pprint.pformat(r)))
    ...
    def _react_to_integ_failure(self, row, column, integ_error_list):
        ...
        '''
        _debug2('_react_to_integ_failure() col={col} {gwt}'
                .format( col = column.index
                       , gwt = row.gwt_path ))
        ...
        if decided.on_integ_failure == decided.NOP:
            _debug3('_react_to_integ_failure() NOP {gwt}', gwt=row.gwt_path)
            return

        if decided.on_integ_failure == decided.RAISE:
            human_msg = (_(
                'Cannot integrate: p4 -c {change_num} integ -i -t {flags}'
                ' {src} {dest}'
                '\n{integ_error_list}')
                .format(
        ...
        _debug3('Integ fallback was={was} fallback={fallback}'
                ' now={now} gwt={gwt}'
               , was      = row.p4_request
               , fallback = decided.integ_fallback
               , now      = new_req
               , gwt      = row.gwt_path )
        row.p4_request = new_req
    ...
    def _do_integ_batch(self, column, row_list):
        ...
        assert column
        assert row_list
        _debug2( '_do_integ_batch col={col} row_ct={row_ct}'
        ...
        * other columns hold any 'p4 integ' actions:
        ...
        for row in self.rows_sorted:
            ...
            for i, cell in enumerate(row.cells):
                ...
                if LOG.isEnabledFor(logging.DEBUG3):
                    LOG.debug3('_decide_p4_requests_post_do_integ()'
    ...
    def _existing_p4filetype(self, row):
        ...
        if not (    row.cells[self._gdest_column.index]
                and row.cells[self._gdest_column.index].discovered ):
            return None
        discovered = row.cells[self._gdest_column.index].discovered

        # Cannot be deleted at head revision.
        if discovered.get('action') and 'delete' in discovered['action']:
            return None
...
# pylint: disable=R0912
    def _decide_one_p4filetype(self, row, prev_p4filetype):
        ...
        if row.p4_request == 'delete':
            return None

        if row.mode == 0o120000:
            return 'symlink'

        # from ebb3597
        if row.mode == 0o160000:
            ### DO WHAT?
            return 'symlink'

        # Force Perforce's x bit to match Git's
        # from ebb3597
        if not row.mode in [ 0o100755, 0o100644 ]:
            raise RuntimeError('_decide_one_p4filetype unexpected row.mode={}'
                               .format( p4gf_util.mode_str(row.mode)))
        git_x = row.mode == 0o100755
...
# pylint: enable=R0912
    ...
    def _decide_p4filetypes_post_do_integ(self):
        ...
        '''
        _debug2('_decide_p4filetypes_post_do_integ')
    ...
    def _unexpand_ktext(self):
        '''
        Any local file that carries the dreaded 'ktext' p4filetype is likely
        polluted with expanded RCS keywords, which in turn change its blob sha1
        and trigger a whole lot of unnecessary 'p4 edit' p4_requests.

        Unexpand where possible.
        '''
        with Timer(DO_UNEXPAND_KTEXT):
            # Build a list of local_paths that need de-ktext-ing
            local_path_list = []
            for row in self.rows_sorted:
                if not (    row.p4filetype
                        and 'k' in p4gf_p4filetype.to_base_mods(
                                                           row.p4filetype)[1] ):
                    continue
            ...
            # de-ktext 'em
            p4gf_util.unexpand_ktext_list( local_path_list
    ...
    def _copy_file_from_git(self, row):
        '''
        Copy one file from Git's internal file blob to correct local_path
        for eventual 'p4 add/edit' + 'p4 submit'.

        NOP for files that already exist. We assume those files passed
        _set_p4_requests_for_local_git_diffs() and matched, no need to
        re-copy such content from Git.

        NOP for rows that are not going to be add/edit/reopen-ed.
        '''
        _debug2('_copy_file_from_git      {row}', row=row)
        # Not in Git?
        if not row.sha1:
            return
        # Not scheduled for add, edit, or reopen?
        if row.p4_request not in ['add', 'edit', None]:
            return
        if row.p4_request == None:# and row.p4filetype == None:
            return
        ...
        _debug2('_copy_file_from_git copy {row}', row=row)
        p4gf_git.cat_file_to_local_file(row.sha1, row.p4filetype, local_path)
    ...
    def _do_batches_add_edit_delete(self):
        ...
        with Timer(DO_BATCH_ADD_EDIT_DELETE):
            ...
            for row in self.rows_sorted:
                if not row.p4_request:
                    continue

                if (    row.p4_request == 'edit'
                    and row.p4filetype
                    and (   row.has_integ()
                         or row.p4filetype == 'symlink')):
                    key = RequestFiletype('edit', None)
                    reopen[row.p4filetype].append(row)
                else:
                    key = RequestFiletype( p4_request = row.p4_request
                                         , p4filetype = row.p4filetype )
            ...
            for key, path_list in req_to_rows.items():
                ...
                cmd = self._batch_add_edit_delete_to_cmd( key.p4_request
                                                        , key.p4filetype
                                                        , path_list)
                _debug2('_do_batches_add_edit_delete() {}', cmd)
...
class RowDecider:
    ...
    def decide(self, row):
        ...
        self.integ_work_queue = \
                deque(col for col in Column.of_col_type( self.m.columns
                                                       , Column.GPARN))
        _debug3('Row.decide() initial integ_work_queue={col}'
        ...
        # Indicate if a decsion was made or not.
        return (self.gdest_cell and self.gdest_cell.decided) or \
                (self.p4jitfp_cell and self.p4jitfp_cell.decided) or \
                (self.git_delta_cell and self.git_delta_cell.decided)
    ...
    def _decide_populate_from(self):
        '''
        ...
        '''
        # LOG.error('### _decide_populate_from() fc={fc} eg={eg} sl={sl} {row}'
        #           .format( fc=1 if self._populate_from_cell() else 0
        #                  , eg=1 if self.row.exists_in_git()   else 0
        #                  , sl=self.m.symlink_in_path(self.row.gwt_path)
        #                  , row=self.row ))
        ...
        if not self.row.exists_in_git():
            return

        # Cannot populate from a file that is deleted at
        # head revision in Perforce.
        if (    self.populate_from_cell.discovered
            and 'action' in self.populate_from_cell.discovered
            and 'delete' in self.populate_from_cell.discovered['action']):
            return
        ...
        _debug3( '_decide_populate_from() col={col} integ -Rbd resolve -at'
    ...
    def _decide_integ_from_column(self, column):
        ...
        '''
        _debug3('Row._decide_integ_from_column() {col}', col=column.index)
        # Skip any already-integrated population source
        if column is self.m._populate_from_column:
            _debug3( 'Row._decide_integ_from_column() {col} == pop_fm. Skipping.'
                   , col=column.index )
            return
        # Or from our destination branch.
        if column.branch == self.m.current_branch:
            _debug3( 'Row._decide_integ_from_column() {col} == curr. Skipping.'
                   , col=column.index )
            return
        ...
        if not src_cell or not src_cell.discovered:
            ...
            _debug3( 'Row._decide_integ_from_column() {col} no src disc.'
                     ' Skipping.'
                    , col=column.index )
            return
        ...
        if not 'endFromRev' in src_cell.discovered:
            _debug3( 'Row._decide_integ_from_column() {col} no src endFromRev.'
                     ' Skipping.'
                    , col=column.index )
            return
        ...
        if self.is_integ_branch_or_delete and cur_is:
            _debug3( 'Row._decide_integ_from_column() {col}'
                     ' double-delete/branch. Skipping.'
                   , col=column.index )
            return
        # Or integs that treat a symlink file as a directory.
        if self.m.symlink_in_path(self.row.gwt_path):
            _debug3( 'Row._decide_integ_from_column() {col} symlink ancestor.'
                     ' Skipping.'
                    , col=column.index)
            return
        ...
        if not self._after_dest_fp(src_cell):
            _debug3( 'Row._decide_integ_from_column() {col} Not after'
                     ' JIT basis. Skipping.'
                    , col=column.index )
            return
        ...
        if (   not r
            ...
            _debug3('Row._decide_integ_from_column() {col} {ri} matrix returned'
                    ' no action. Skipping.'
                   , col=column.index
        ...
        _debug3( 'Row._decide_integ_from_column() col={col} decided={decided}'
               , col     = column.index
    ...
    def _after_dest_fp(self, src_cell):
        ...
        if not self.p4jitfp_cell:
            _debug3('_after_dest_fp() True no FP basis. ')
            return True
        ...
        if (   'depotFile' not in fp
            or 'depotFile' not in src
            or fp['depotFile'] != src['depotFile']):
            _debug3('_after_dest_fp() True src={src} not from'
                    ' JIT FP basis={fp}. '
                    .format(src=src.get('depotFile')
                           ,fp =fp .get('depotFile')))
            return True
        ...
        if ( 'rev' not in fp
            ...
            _debug3('_after_dest_fp() True src={src} > '
                    ' JIT FP basis={fp}. '
        ...
        _debug3('_after_dest_fp() True src={src_d}#{src_r} <= '
    ...
    def _decide_jit_if(self):
        ...
        if self.has_integ:
            _debug3('_decide_jit_if() no: has_integ')
            return
        ...
        if not (    self.git_delta_cell
            ...
            _debug3('_decide_jit_if() no: no git delta from prev commit')
            return

        # No need to JIT-branch a file that already exists in destination.
        if _gdest_cell_exists_in_p4(self.gdest_cell):
            _debug3('_decide_jit_if() no: already exists in p4 destination')
            return

        # Cannot JIT-branch a file that has no source.
        if not self._p4jitfp_exists():
            _debug3('_decide_jit_if() no: does not exist in JIT FP basis')
            return


        if self.m.symlink_in_path(self.row.gwt_path):
            _debug3('_decide_jit_if() no: symlink in path')
            return
        ...
        if (   not r
            or not ((r.integ_flags != None) or r.fallback) ):
            _debug3('_decide_jit_if() no: integ matrix returned'
                    ' no integ or fallback')
            return

        self.p4jitfp_cell.decided = Decided.from_integ_matrix_row(r, ri)
        _debug3('_decide_jit_if() JIT: {}'.format(self.p4jitfp_cell.decided))
        if r.integ_flags != None:
            self.has_integ = True
    ...
    def _p4jitfp_exists(self):
        ...
        '''
        return (    self.p4jitfp_cell
                and self.p4jitfp_cell.discovered
                and 'depotFile'  in self.p4jitfp_cell.discovered
                and 'action'     in self.p4jitfp_cell.discovered
                and 'delete' not in self.p4jitfp_cell.discovered['action'])
    ...
    def _apply_git_delta(self):
        ...
        '''
        _apply_git_delta_cell( gdest_cell     = self.gdest_cell
                             , git_delta_cell = self.git_delta_cell )
...
def _gdest_cell_deleted_at_head(gdest_cell):
    '''
    Does this cell.discovered have an 'action' of 'delete'?
    '''
    return (    gdest_cell
            and gdest_cell.discovered
            and 'action' in gdest_cell.discovered
            and 'delete' in gdest_cell.discovered['action'])
...
def _apply_git_delta_cell(gdest_cell, git_delta_cell):
    '''
    If git-fast-export or git-diff-tree says to Add/Modify/Delete
    this GWT path, then do so.
    ...
    integ decision table already factors in Git actions when integrating.
    '''
    if not (    git_delta_cell
            and git_delta_cell.discovered
            and git_delta_cell.discovered.get('git-action')):
        return

    exists = True if (        _gdest_cell_exists_in_p4(gdest_cell)
                      and not _gdest_cell_deleted_at_head(gdest_cell)) \
             else False
    git_action = git_delta_cell.discovered.get('git-action')
    _debug3('_apply_git_delta_cell() e={e} g={g} '.format(e=exists, g=git_action))
    action = _DT[exists][git_action]
    if not action:
        return
