...
"""Script to copy all keys from //P4GF_DEPOT/users/*/keys/* to the SSH
...
# lack of quote marks around {} parameter
# OK
...
def update_last_change(p4, value):
    ...
    try:
        v = int(value)
        if v < 1:
            raise RuntimeError(_("invalid change number '{}'").format(value))
...
def read_key_type(key):
    ...
    try:
        ...
        _print_warn(_("apparently invalid SSH key '{}' caused '{}'").format(key, e))
    except UnicodeDecodeError:
        _print_warn(_("error decoding SSH key type for key '{}'").format(key), error=True)
        return None
...
def ssh_key_to_fingerprint(key):
    ...
    try:
        ...
        _print_warn(_("failed to hash SSH key: '{}'").format(e))
    return None
...
def ssh_key_add(p4, depot_path, keys, action=None):
    ...
    if not user:
        _print_warn(_('Could not extract user name from unrecognized depot path:') +
                    ' {}'.format(depot_path))
        return
    if not fp:
        _print_warn(_("File '{}' does not conform to a valid SSH key, ignoring...")
                    .format(depot_path))
        return
    if not action:
        action = _ADD
...
def update_by_changes(p4):
    ...
    if latest_change <= last_change:
        ...
        _print_debug(_('NOP. Changes to {path}={ch} < counter {counter}={ct}').format(
            path=KEYS_PATH,
            ch=latest_change,
    ...
    for change in changes:
        _print_debug(_('processing change @{}: {}').format(change.change, change.description))
        ...
        for detail in change.files:
            ...
            if not KEYPATH_RE.search(name):
                ...
                continue
            _print_debug(_('file {}, action {}').format(name, detail.action))
            if detail.action in [FA.ADD, FA.MOVE_ADD, FA.BRANCH]:
                ...
                _print_warn(_("unhandled change type '{}'").format(detail.action))
...
def rebuild_all_keys(p4):
    ...
    if not latest_change:
        _print_warn(_("No files found in '{}'").format(KEYS_PATH))
        return
    _print_debug(_('rebuilding all keys through change {}').format(latest_change))
    keys = read_ssh_configuration()
    ...
    if files:
        for fi in files:
            _print_debug(_('adding file {}').format(fi['depotFile']))
